...
def generate_license_footer(tool_name: str) -> str:
    """
    Generate the standard license and attribution footer for reports.
    
    Args:
        tool_name: Name of the tool generating the report
        
    Returns:
        Formatted license footer text
    """
    fetch_date = get_fetch_date()
    analysis_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    return f"""## License and Attribution
...
def write_markdown_report(file_path: str, content: str) -> None:
    """
    Write a markdown report to file with UTF-8 encoding.
