...
async function loadConfig(currentSite, elements) {
    debugLog('[DEBUG] loadConfig開始:', currentSite);
    try {
        const configPath = `./config.yml`;
        debugLog('[DEBUG] config.yml fetch開始:', configPath);
        
        const response = await fetch(configPath);
        debugLog('[DEBUG] config.yml fetch結果:', response.ok, response.status);
        if (!response.ok) {
        ...
        debugLog('[DEBUG] YAML parse開始');
        const config = yamlLoad(yamlText);
        debugLog('[DEBUG] YAML parse完了:', config ? 'success' : 'failed');
...
async function handleOnClickLoadData(elements) {
    debugLog('[DEBUG] handleOnClickLoadData開始', { 
        isLoading, 
        embedder: !!embedder, 
    ...
    if (!embedder && window?.embedder) {
        ...
        debugLog('[DEBUG] embedder変数をwindow.embedderで同期');
    }
    
    debugLog('[DEBUG] データ読み込み開始');
    isLoading = true;
    elements.loadDataBtn.disabled = true;
    elements.loadingProgress.classList.add('loading');
    
    // Calculate total progress steps
    debugLog('[DEBUG] メタデータ取得開始:', CONFIG.VECTOR_STORE_META_PATH);
    ...
    debugLog('[DEBUG] メタデータ取得完了:', metadata);
    ...
    debugLog('[DEBUG] 総ステップ数計算中:', { totalSteps, num_parts: metadata.num_parts });
    ...
    try {
        ...
        debugLog('[DEBUG] ベクトルストアパート読み込み開始:', metadata.num_parts);
...
async function handleOnLoad(currentSite) {
    ...
    try {
        debugLog('[DEBUG] pipelineImport開始:', typeof pipelineImport);
        const { pipeline } = await pipelineImport();
        debugLog('[DEBUG] pipeline取得完了:', typeof pipeline);
        
        debugLog('[DEBUG] embedder初期化開始:', CONFIG.EMBEDDING_MODEL);
        embedder = await pipeline('feature-extraction', CONFIG.EMBEDDING_MODEL);
        debugLog('[DEBUG] embedder初期化完了:', typeof embedder);
