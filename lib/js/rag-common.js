...
// Dynamic import for embedding pipeline (browser vs Node.js)
let pipelineImport;
if (typeof window !== 'undefined') {
    // Browser environment
    pipelineImport = () => import('https://cdn.jsdelivr.net/npm/@xenova/transformers@2.6.0/dist/transformers.min.js');
} else {
    // Node.js environment
    pipelineImport = () => import('@xenova/transformers');
}
...
async function loadConfig(currentSite, elements) {
    //console.log('[DEBUG] loadConfig開始:', currentSite);
    try {
        const configPath = `./config.yml`;
        //console.log('[DEBUG] config.yml fetch開始:', configPath);
        
        const response = await fetch(configPath);
        //console.log('[DEBUG] config.yml fetch結果:', response.ok, response.status);
        if (!response.ok) {
            throw new Error(`Failed to load config: ${response.status}`);
        }
        
        const yamlText = await response.text();
        //console.log('[DEBUG] YAML text length:', yamlText.length);
        //console.log('[DEBUG] YAML parse開始');
        const config = yamlLoad(yamlText);
        //console.log('[DEBUG] YAML parse完了:', config ? 'success' : 'failed');
        ...
        //console.log('[DEBUG] CONFIG構築完了');
        return CONFIG;
    } catch (error) {
        //console.error('[DEBUG] loadConfig エラー:', error);
        throw error;
    }
}
...
async function handleOnClickLoadData(elements) {
    //console.log('[DEBUG] handleOnClickLoadData開始', { 
    //    isLoading, 
    //    embedder: !!embedder, 
    ...
    if (!embedder && window?.embedder) {
        ...
        //console.log('[DEBUG] embedder変数をwindow.embedderで同期');
    }
    
    //console.log('[DEBUG] データ読み込み開始');
    isLoading = true;
    elements.loadDataBtn.disabled = true;
    elements.loadingProgress.classList.add('loading');
    
    // Calculate total progress steps
    //console.log('[DEBUG] メタデータ取得開始:', CONFIG.VECTOR_STORE_META_PATH);
    ...
    //console.log('[DEBUG] メタデータ取得完了:', metadata);
    ...
    //console.log('[DEBUG] 総ステップ数計算中:', { totalSteps, num_parts: metadata.num_parts });
    ...
    try {
        ...
        //console.log('[DEBUG] ベクトルストアパート読み込み開始:', metadata.num_parts);
        ...
        for (let partIndex = 1; partIndex <= metadata.num_parts; partIndex++) {
            ...
            //console.log(`[DEBUG] パート${partIndex}読み込み開始:`, partPath);
            
            const response = await fetch(partPath);
            if (!response.ok) {
                //console.log(`[DEBUG] パート${partIndex}取得失敗:`, response.status);
                throw new Error(`Failed to load part ${partIndex}: ${response.status}`);
            }
            
            //console.log(`[DEBUG] パート${partIndex}フェッチ完了、解凍開始`);
            const arrayBuffer = await response.arrayBuffer();
            const decompressed = pako.inflate(arrayBuffer, { to: 'string' });
            const partData = JSON.parse(decompressed);
            //console.log(`[DEBUG] パート${partIndex}解析完了:`, { documents: partData.documents?.length });
...
async function handleOnLoad(currentSite) {
    ...
    try {
        //console.log('[DEBUG] pipelineImport開始:', typeof pipelineImport);
        const { pipeline } = await pipelineImport();
        //console.log('[DEBUG] pipeline取得完了:', typeof pipeline);
        
        //console.log('[DEBUG] embedder初期化開始:', CONFIG.EMBEDDING_MODEL);
        embedder = await pipeline('feature-extraction', CONFIG.EMBEDDING_MODEL);
        //console.log('[DEBUG] embedder初期化完了:', typeof embedder);
