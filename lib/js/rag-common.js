...
// Every koteitan site is served from the single origin https://koteitan.github.io/,
// so localStorage keys must be namespaced by repository name to avoid collisions.
// All settings for this app live in one JSON object under "googolbook-lm:state".
const LS_NAMESPACE = 'googolbook-lm';
const lsKey = (name) => LS_NAMESPACE + ':' + name;

// Read the whole state object. When the namespaced key is absent, fall back to
// the legacy un-namespaced keys (they are read only, never removed).
function loadState(storage) {
    if (!storage) return {};
    try {
        const raw = storage.getItem(lsKey('state'));
        if (raw !== null && raw !== undefined) {
            const parsed = JSON.parse(raw);
            if (parsed && typeof parsed === 'object') return parsed;
        }
    } catch (error) {
        //console.warn('Could not read state from localStorage:', error);
    }
    return loadLegacyState(storage);
}

// Build a state object out of the pre-namespace keys ('lastQuery', 'ragSettings').
function loadLegacyState(storage) {
    const state = {};
    if (!storage) return state;
    try {
        const lastQuery = storage.getItem('lastQuery');
        if (lastQuery !== null && lastQuery !== undefined) state.lastQuery = lastQuery;
    } catch (error) {
        //console.warn('Could not read legacy lastQuery:', error);
    }
    try {
        const ragSettings = storage.getItem('ragSettings');
        if (ragSettings !== null && ragSettings !== undefined) {
            const parsed = JSON.parse(ragSettings);
            if (parsed && typeof parsed === 'object') state.ragSettings = parsed;
        }
    } catch (error) {
        //console.warn('Could not read legacy ragSettings:', error);
    }
    return state;
}

// Merge patch into the stored state and write it back to the namespaced key.
function saveState(storage, patch) {
    if (!storage) return;
    try {
        const state = loadState(storage);
        Object.assign(state, patch);
        storage.setItem(lsKey('state'), JSON.stringify(state));
    } catch (error) {
...
function handleOnClickSend(elements) {
    ...
    saveState(elements.localStorage, { lastQuery: query });
...
function saveSettingsToLocalStorage(elements) {
    try {
        ...
        saveState(elements.localStorage, { ragSettings: settings });
    } catch (error) {
...
function loadSettingsFromLocalStorage(elements) {
    try {
        const settings = loadState(elements.localStorage).ragSettings;
        if (settings) {
            if (settings.baseUrl) {
...
async function handleOnLoad(currentSite) {
    ...
    try {
        const lastQuery = loadState(elements.localStorage).lastQuery;
