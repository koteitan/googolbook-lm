...
// rag-common.jsをNode.js用に変換して一時ファイル作成
async function createNodeJsVersion() {
    ...
    // ブラウザ用のimportをNode.js用に変更
    const modifiedCode = ragCommonCode
        // yamlのimportを置換
        .replace(/import.*?from.*?['"]https:\/\/cdn\.skypack\.dev\/js-yaml@.*?['"];?/g, 'import { load as yamlLoad } from "js-yaml";')
        // pakoのimportを置換
        .replace(/import.*?from.*?['"]https:\/\/cdn\.skypack\.dev\/pako@.*?['"];?/g, 'import pako from "pako";')
        // transformersのimportを先頭に追加
        .replace(/^/, 'import { pipeline } from "@xenova/transformers";\n')
        // pipelineImportの条件分岐を削除して直接定義（pipelineをimportから参照）
        .replace(/let pipelineImport;[\s\S]*?} else {\s*\/\/ Node\.js environment[\s\S]*?}/g, 'const pipelineImport = () => Promise.resolve({ pipeline });')
        // alert関数を追加
        .replace(/^/, 'const alert = (msg) => console.log("[alert]", msg);\n')
        // exportを削除
        .replace(/export\s*\{[^}]+\};?/g, '');
    
    // 末尾にexportを追加
    const finalCode = modifiedCode + '\nexport { handleOnLoad, handleOnClickLoadData };';
    
    // 一時ファイルを作成
    const tempPath = path.join(__dirname, 'temp-rag-common.mjs');
    
    // デバッグ用：import文とpipelineImportの確認（無効化）
    // console.log('[DEBUG] 生成されたコードの最初の15行:');
    // const lines = finalCode.split('\n');
    // lines.slice(0, 15).forEach((line, i) => {
    //     console.log(`${i+1}: ${line}`);
    // });
    
    // pipelineに関連する行を検索（無効化）
    // console.log('\n[DEBUG] pipeline関連の行:');
    // lines.forEach((line, i) => {
    //     if (line.includes('pipeline') || line.includes('@xenova/transformers')) {
    //         console.log(`${i+1}: ${line}`);
    //     }
    // });
...
async function runWalkthroughTest() {
    ...
    try {
        ...
        try {
            // 動的import
            const { handleOnLoad, handleOnClickLoadData } = await import(tempPath);
