    ...
    try {
        ...
        const context = searchResults.map(result => {
            if (result.isDocumentGroup && result.allChunks && result.allChunks.length > 1) {
                // For document groups, merge all chunks with overlap removal
                console.log(`Processing document group: ${result.title} (${result.allChunks.length} chunks)`);
                
                const mergedContent = mergeDocumentChunks(result.allChunks);
