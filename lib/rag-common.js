...
const PROVIDER_CONFIG = {
    openai: {
        ...
        apiKeyPlaceholder: 'OpenAI API キーを入力',
        apiKeyHelp: '<a href="https://platform.openai.com/api-keys" target="_blank">OpenAI API key</a>を入力してください',
        baseUrlPlaceholder: 'https://api.openai.com/v1'
    },
    claude: {
        name: 'Claude (Anthropic)',
        baseUrl: 'https://api.anthropic.com/v1',
        apiKeyPlaceholder: 'Anthropic API キーを入力',
        apiKeyHelp: '<a href="https://console.anthropic.com/" target="_blank">Anthropic API key</a>を入力してください',
        baseUrlPlaceholder: 'https://api.anthropic.com/v1'
    },
    gemini: {
        name: 'Gemini (Google)',
        baseUrl: 'https://generativelanguage.googleapis.com/v1beta',
        apiKeyPlaceholder: 'Google API キーを入力',
        apiKeyHelp: '<a href="https://makersuite.google.com/app/apikey" target="_blank">Google API key</a>を入力してください',
        baseUrlPlaceholder: 'https://generativelanguage.googleapis.com/v1beta'
    },
    openrouter: {
        name: 'OpenRouter',
        baseUrl: 'https://openrouter.ai/api/v1',
        apiKeyPlaceholder: 'OpenRouter API キーを入力',
        apiKeyHelp: '<a href="https://openrouter.ai/keys" target="_blank">OpenRouter API key</a>を入力してください',
        baseUrlPlaceholder: 'https://openrouter.ai/api/v1'
    },
    'azure-openai': {
        name: 'Open AI Compatible API',
        baseUrl: 'https://your-resource.openai.azure.com',
        apiKeyPlaceholder: 'Open AI Compatible API キーを入力',
        apiKeyHelp: '<a href="#" target="_blank">Open AI Compatible API key</a>を入力してください',
 ...
 */
function getRepresentativeContentFromChunks(curid, vectorStoreParts, sizeLimit = null) {
 ...
 */
function processContentForLLM(content, refNumber, title, type = 'content', sizeLimit = null, curid = null, vectorStoreParts = null) {
 ...
 */
function processSearchResultContent(result, refNumber, type = 'content', sizeLimit = null, displayLimit = null) {
    let rawContent;
...
function extractBodyContent(result, refNumber) {
    ...
    if (result.isDocumentGroup && result.allChunks && result.allChunks.length > 1) {
        ...
        if (estimatedTokens > documentSizeThreshold) {
            ...
            const contextSize = contextSizeTokens * CHARS_PER_TOKEN;
            rawContent = processRepresentativeChunk(result.allChunks, result.bestChunk, contextSize);
        } else {
...
function calculateRequiredTokens(titleResults, bodyResults, prompt_document_num, prompt_document_num_body, userQuery) {
    // System prompt template (rough estimate)
    const systemPromptTemplate = `あなたは「巨大数論」に特化したAI助手です。以下のコンテキスト情報を参考にして、ユーザーの質問に正確かつ詳細に答えてください。
...
ユーザーの質問: `;
    ...
    if (prompt_document_num > 0) {
        ...
        for (let i = 0; i < prompt_document_num; i++) {
            ...
            const processedResult = processSearchResultContent(result, i + 1, "title");
    ...
    if (prompt_document_num_body > 0) {
        ...
        for (let i = 0; i < prompt_document_num_body; i++) {
            ...
            const processedResult = processSearchResultContent(result, prompt_document_num + i + 1, "body");
            if (!processedResult || !processedResult.llmContent) {
    ...
    const finalSize = totalSystemPromptSize + userQuery.length;
    
    console.log(`🧮 Token calculation debug:`, {
        systemPromptTemplate: systemPromptTemplate.length,
        estimatedContextSize: estimatedContextSize,
        userQuery: userQuery.length,
        totalSystemPromptSize: totalSystemPromptSize,
        finalSize: finalSize,
        isNaN: isNaN(finalSize)
    });
    
    return finalSize;
}

function findOptimalDocumentNumbers(titleResults, bodyResults, userQuery, tokenLimit) {
    const maxDocs = Math.min(titleResults.length, bodyResults.length);
    
    // Create token_size[prompt_document_num] table
    const token_size = {};
    console.log(`🔍 Finding optimal document numbers:`);
    console.log(`  📚 Available: ${titleResults.length} title ranking results, ${bodyResults.length} body ranking results`);
    console.log(`  🎯 Max docs per type: ${maxDocs} (limited by smaller ranking)`);
    console.log(`  🚫 Token limit: ${tokenLimit.toLocaleString()} chars`);
    ...
    for (let prompt_document_num = maxDocs; prompt_document_num >= 1; prompt_document_num--) {
        console.log(`  🧮 Trying prompt_document_num=${prompt_document_num} (top ${prompt_document_num} from each ranking)...`);
        const requiredTokens = calculateRequiredTokens(
            titleResults, bodyResults, prompt_document_num, prompt_document_num, userQuery
        );
        
        token_size[prompt_document_num] = requiredTokens;
        console.log(`  📏 token_size[${prompt_document_num}] = ${requiredTokens.toLocaleString()} chars | Limit: ${tokenLimit.toLocaleString()} chars`);
        
        if (requiredTokens <= tokenLimit) {
            console.log(`  ✅ Selected prompt_document_num=${prompt_document_num}: title ranking top ${prompt_document_num} + body ranking top ${prompt_document_num} (${requiredTokens.toLocaleString()} chars)`);
            return {
            ...
            console.log(`  ❌ Exceeds limit, trying smaller prompt_document_num...`);
        }
    }
    
    // Even 1 document exceeds limit
    console.log(`  ⚠️ Even 1 document per type exceeds limit!`);
    return {
        titleDocNum: 1,
        bodyDocNum: 1,
...
function displayRAGResults(elements, bodyResults, titleResults) {
    ...
    if (titleResults && titleResults.length > 0) {
        const titleHTML = titleResults.map((result, index) => {
            ...
            const { displayContent } = processSearchResultContent(result, refNumber, 'title', null, 1000);
    ...
    if (bodyResults && bodyResults.length > 0) {
        const bodyHTML = bodyResults.map((result, index) => {
            ...
            // Use common processing function with 1000 char display limit
            const { displayContent } = processSearchResultContent(result, refNumber, 'body', null, 1000);
    ...
    if (window.lastDocumentSelection) {
        const info = window.lastDocumentSelection;
        console.log('📊 UI Display Info:', info);
        const selectionInfoHTML = `
        <div class="document-selection-info">
            <h3>📊 Dynamic Document Selection</h3>
            <p><strong>Token-based optimization:</strong> Selected prompt_document_num=${info.selectedPromptDocumentNum} (${info.estimatedTokens && CHARS_PER_TOKEN ? Math.round(info.estimatedTokens / CHARS_PER_TOKEN).toLocaleString() : 'N/A'} tokens, ${info.estimatedTokens ? info.estimatedTokens.toLocaleString() : 'N/A'} chars)</p>
            <p><strong>Documents per type:</strong> Title ranking top ${info.selectedPromptDocumentNum}, Body ranking top ${info.selectedPromptDocumentNum}</p>
            <p><strong>Available:</strong> ${info.availableTitleDocs} title results, ${info.availableBodyDocs} body results (max possible: ${info.maxPossibleDocs})</p>
        </div>
        `;
        resultsHTML = selectionInfoHTML + resultsHTML;
    }
...
async function generateAIResponse(elements, query, bodyResults, titleResults, apiKey, keywordExtractionResult = null) {
    ...
    try {
        ...
        const optimalDocs = findOptimalDocumentNumbers(titleResults, bodyResults, query, tokenLimit);
        ...
        console.log('Dynamic document adjustment:', window.lastDocumentSelection);
        ...
        const titleContext = selectedTitleResults.map(result => {
            ...
            const { llmContent } = processSearchResultContent(result, refNumber, 'title');
        ...
        const bodyContext = selectedBodyResults.map(result => {
            ...
            // Use common processing function
            const { llmContent } = processSearchResultContent(result, refNumber, 'body');
...
${combinedContext}`;
        ...
        const aiResponse = await callLLMAPI(baseUrl, apiKey, currentModel, systemPrompt, query);
        ...
        if (error.response && error.response.status === 429) {
            ...
            elements.responseWindow.innerHTML = `
                <div class="error rate-limit-error">
                    <h3>⏳ ${providerName} APIレート制限に達しました</h3>
...
async function callLLMAPI(baseUrl, apiKey, model, systemPrompt, userQuery) {
...
function updateUIForProvider(elements) {
    ...
    if (apiKeyHelp) {
        apiKeyHelp.innerHTML = config.apiKeyHelp;
    }
