        ...
        vectorStore = {
            ...
            searchByTitle: async function(queryEmbedding, k = CONFIG.FINAL_RESULT_COUNT) {
                    ...
                    for (let partIndex = 0; partIndex < this.titleParts.length; partIndex++) {
                        ...
                        for (let docIndex = 0; docIndex < titlePart.documents.length; docIndex++) {
                            ...
                            if (doc.curid === "345" || (doc.metadata && doc.metadata.curid === "345")) {
                                console.log(`🎯 DEBUG: グラハム数 (curid=345) - Similarity: ${titleSimilarity.toFixed(6)}, Title: "${doc.metadata?.title}"`);
                            }
                ...
                for (let i = 0; i < Math.min(10, titleSimilarities.length); i++) {
                    ...
                    console.log(`  ${i+1}. ${result.metadata?.title || 'No title'} (curid: ${result.curid}) - Score: ${result.score.toFixed(6)}`);
                }
...
async function extractSearchKeywords(query, apiKey) {
    ...
    const keywordPrompt = `
あなたは巨大数学（googology）専門の検索システムのキーワード抽出エキスパートです。

以下のユーザークエリから、巨大数学のRAG検索を行うために最も重要な専門用語のみを最大5個抽出してください。

ユーザークエリ: "${query}"

抽出ルール:
1. 巨大数学・数学・計算機科学の専門用語・固有名詞のみを抽出
2. 以下のような専門用語を優先:
   - 巨大数の名称（グラハム数、ふぃっしゅ数、BM2.2、TREE(3)など）
   - 数学記法・システム（バシク行列、アッカーマン関数、ヴェブレン階層など）
   - 人名（バシク、フィッシュ、グラハムなど）
   - 数学概念（順序数、超限、階層など）
3. 一般的な単語は除外:
   - 疑問詞（とは、について、なぜ、どのように、違い）
   - 一般名詞（アルゴリズム、システム、数、計算、定義、理論）
   - 動詞・形容詞（大きい、小さい、作る、計算する）
4. 英数字の専門記号・略語は維持（BM2.2、TREE(3)、φ(Ω,0)など）
5. 最大5個、最低1個のキーワード
...
async function processSearchAndResponse(elements, query) {
    ...
    try {
        ...
        const keywordExtractionResult = await extractSearchKeywords(query, apiKey);
        ...
        const searchResults = await performMultiKeywordSearch(keywordExtractionResult.keywords);
