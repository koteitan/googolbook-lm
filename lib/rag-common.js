...
// Load and parse compressed JSONL file (with XML fallback)
async function loadCompressedXML(xmlGzPath, updateProgress) {
    try {
        // Try JSONL first
        const jsonlGzPath = xmlGzPath.replace('.xml.gz', '.jsonl.gz');
        //console.log('Attempting to load JSONL from:', jsonlGzPath);
        updateProgress(0, 'Downloading JSONL file...');
        
        let response = await fetch(jsonlGzPath);
        let isJSONL = response.ok;
        //console.log('JSONL response status:', response.status, 'ok:', response.ok);
        
        if (!isJSONL) {
            // Fallback to XML
            //console.log('JSONL not found, falling back to XML:', xmlGzPath);
            updateProgress(0, 'Downloading XML file...');
            response = await fetch(xmlGzPath);
            if (!response.ok) {
        ...
        updateProgress(50, isJSONL ? 'Decompressing JSONL...' : 'Decompressing XML...');
        ...
        if (isJSONL) {
            ...
            //console.log(`XML loaded successfully: ${pages.length} pages, ${(text.length / 1024 / 1024).toFixed(1)} MB`);
        }
...
// Get page content from XML or JSONL by curid
function getPageFromXML(curid) {
    ...
    // Check if it's JSONL data (plain object) or XML (DOM element)
    if (page.title !== undefined && page.text !== undefined) {
        // JSONL format
        ...
        vectorStore = {
            ...
            searchByTitle: async function(queryEmbedding, k = CONFIG.FINAL_RESULT_COUNT) {
                    ...
                    for (let partIndex = 0; partIndex < this.titleParts.length; partIndex++) {
                        ...
                        for (let docIndex = 0; docIndex < titlePart.documents.length; docIndex++) {
                            ...
                            if (doc.curid === "345" || (doc.metadata && doc.metadata.curid === "345")) {
                                const xmlPageData = xmlData ? getPageFromXML(doc.curid) : null;
                ...
                for (let i = 0; i < Math.min(10, titleSimilarities.length); i++) {
                    ...
                    const xmlPageData = xmlData ? getPageFromXML(result.curid) : null;
