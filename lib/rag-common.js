...
async function generateAIResponse(elements, query, bodyResults, titleResults, apiKey) {
    ...
    try {
        ...
        const titleContext = titleResults.map(result => {
            ...
            console.log(`📄 [${refNumber}] Title result: "${result.title}"`);
            console.log(`📊 [${refNumber}] Original content size: ${result.content.length.toLocaleString()} chars`);
            
            let rawContent = result.content;
            
            // Apply size limit check to title results too
            if (result.content.length > DOCUMENT_SIZE_LIMIT) {
                console.log(`🎯 [${refNumber}] Size limit check: ${result.content.length.toLocaleString()} vs ${DOCUMENT_SIZE_LIMIT.toLocaleString()}`);
                console.log(`🔪 [${refNumber}] ❌ TOO LARGE! Truncating title content (limit: ${DOCUMENT_SIZE_LIMIT.toLocaleString()})`);
                rawContent = result.content.substring(0, DOCUMENT_SIZE_LIMIT) + '\n\n[Content truncated due to size limit]';
                console.log(`✂️ [${refNumber}] Truncated result: ${rawContent.length.toLocaleString()} chars`);
            } else {
                console.log(`🎯 [${refNumber}] Size limit check: ${result.content.length.toLocaleString()} vs ${DOCUMENT_SIZE_LIMIT.toLocaleString()}`);
            ...
            const reduction = rawContent.length - cleanedContent.length;
