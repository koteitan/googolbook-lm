...
const MODEL_LIMITS = {
    // OpenAI models
    'o3-pro': 200000 * 3.5,        // 200k tokens → ~700k chars
    'o4-mini': 200000 * 3.5,       // 200k tokens → ~700k chars
    ...
    'claude-opus-4-20250514': 200000 * 3.5,      // ~200k tokens → ~700k chars
    'claude-sonnet-4-20250514': 200000 * 3.5,    // ~200k tokens → ~700k chars
    'claude-3-7-sonnet-20250219': 200000 * 3.5,  // ~200k tokens → ~700k chars
    'claude-3-5-sonnet-20241022': 200000 * 3.5,  // ~200k tokens → ~700k chars
    'claude-3-5-haiku-20241022': 200000 * 3.5,   // ~200k tokens → ~700k chars
    'claude-3-haiku-20240307': 200000 * 3.5,     // ~200k tokens → ~700k chars
    
    // Gemini models (estimated token limits)
    'gemini-2.5-pro': 2000000 * 3.5,    // ~2M tokens → ~7M chars
    'gemini-2.5-flash': 1000000 * 3.5,  // ~1M tokens → ~3.5M chars
    
    // OpenRouter models (same as underlying models)
    'openrouter/openai/chatgpt-4o-latest': 128000 * 3.5,    // 128k tokens → ~448k chars
    'openrouter/openai/o3-pro': 200000 * 3.5,               // 200k tokens → ~700k chars
    'openrouter/openai/o4-mini': 200000 * 3.5,              // 200k tokens → ~700k chars
    'openrouter/anthropic/claude-opus-4': 200000 * 3.5,     // ~200k tokens → ~700k chars
    'openrouter/anthropic/claude-sonnet-4': 200000 * 3.5,   // ~200k tokens → ~700k chars
    'openrouter/anthropic/claude-3.7-sonnet': 200000 * 3.5, // ~200k tokens → ~700k chars
    'openrouter/anthropic/claude-3.5-sonnet': 200000 * 3.5, // ~200k tokens → ~700k chars
    'openrouter/google/gemini-2.5-pro': 2000000 * 3.5,      // ~2M tokens → ~7M chars
    'openrouter/google/gemini-2.5-flash': 1000000 * 3.5,    // ~1M tokens → ~3.5M chars
    
    // Azure OpenAI models (same as OpenAI models)
    'azure/gpt-4o': 128000 * 3.5,        // 128k tokens → ~448k chars
    'azure/gpt-4o-mini': 128000 * 3.5,   // 128k tokens → ~448k chars
    'azure/gpt-4-turbo': 128000 * 3.5,   // 128k tokens → ~448k chars
    'azure/gpt-4': 8192 * 3.5,           // 8k tokens → ~28k chars
    'azure/gpt-3.5-turbo': 16384 * 3.5,  // 16k tokens → ~57k chars
};
...
function extractBodyContent(result, refNumber) {
    ...
    if (result.isDocumentGroup && result.allChunks && result.allChunks.length > 1) {
        ...
        // Always merge all chunks - no size limitation
