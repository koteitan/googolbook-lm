                ...
                if (this.titleParts && this.titleParts.length > 0) {
                    ...
                    for (let partIndex = 0; partIndex < this.titleParts.length; partIndex++) {
                        ...
                        for (let docIndex = 0; docIndex < titlePart.documents.length; docIndex++) {
                            ...
                            const titleSimilarity = cosineSimilarity(queryEmbedding, doc.embedding);
...
async function generateAIResponse(elements, query, bodyResults, titleResults, apiKey) {
    try {
        ...
        // Create context from body results
        ...
        // Create context from title results
...
Use the following context to answer the user's question. The context includes both title-based and content-based search results. If the context doesn't contain enough information to answer the question, say so clearly.
