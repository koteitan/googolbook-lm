...
function displayLLMPrompt(systemPrompt, userQuery, promptSize = null, keywordExtractionResult = null) {
    ...
    if (SHOW_LLM_PROMPT && debugSection && debugInfoContent && systemPromptContent && userQueryContent) {
...
`;
        ...
        debugInfoContent.innerHTML = `<pre>${debugInfo}</pre>`;
...
async function callLLMAPI(baseUrl, apiKey, model, systemPrompt, userQuery) {
    ...
    console.error(`DEBUG: Model="${model}", Provider="${provider}", BaseURL="${baseUrl}"`);
...
function updateUIForProvider(elements) {
    ...
    console.error(`DEBUG updateUIForProvider: Model="${selectedModel}", Provider="${provider}", CurrentBaseURL="${elements.baseUrl.value}", NewBaseURL="${config.baseUrl}"`);
    if (elements.baseUrl.value === '' || 
        ...
        console.error(`DEBUG: BaseURL updated from "${elements.baseUrl.value}" to "${config.baseUrl}"`);
        elements.baseUrl.value = config.baseUrl;
    } else {
        console.error(`DEBUG: BaseURL NOT updated - current value "${elements.baseUrl.value}" not in default list`);
    }
}
